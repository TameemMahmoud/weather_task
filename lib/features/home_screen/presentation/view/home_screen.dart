...
class HomeScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<HomeCubit, HomeStates>(builder: (context, state) {
      ...
      return Scaffold(
        body: state is GetCurrentWeatherLoadingState || cubit.currentWeather == null
            ? const Center(
          ...
          physics: const BouncingScrollPhysics(),
              child: Column(
                  children: <Widget>[
                    ...
                    state is GetFiveDaysWeatherLoadingState || cubit.fiveDaysModel == null? const Center(
                      child: CircularProgressIndicator.adaptive(),
                    ) : SizedBox(
                      height: 230,
                      child: ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          return  FiveDaysWidget(data: cubit.fiveDaysModel!.list[index],);
                        },
                      ),
                    ),
                    const SizedBox(height: 20,),
                    Text('5-days forecast by chart', style: AppFonts.headlineStyle.copyWith(color: AppColors.mainColor),),
                    state is GetFiveDaysWeatherLoadingState || cubit.fiveDaysModel == null? const Center(
                      child: CircularProgressIndicator.adaptive(),
                    ) : Padding(
                      padding: const EdgeInsets.all(AppConstants.pagePadding),
                      child: MyChart(cubit: cubit,),
                    ),
                    const SizedBox(height: 20,),
                  ],
                ),
